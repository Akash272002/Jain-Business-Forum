...
<div class="ms-content-wrapper"> 
      <div class="row">
       ...
       <form class="needs-validation" id="Form" action="<?php echo base_url('update/'. (isset($Jobcategory->jobcatergoryId) ? 'updateJobCategory' : 'insertJobCategory')); ?>" method="post" autocomplete="off">
    <input name="jobcatergoryId" id="jobcatergoryId" value="<?= isset($Jobcategory->jobcatergoryId) ? $Jobcategory->jobcatergoryId : '' ?>" type="hidden"> 
    <div class="form-row">
        <div class="col-md-4">
            <label for="name">Job Category Name <span class="required" style="color: red">*</span></label>
            <div class="input-group">
                <input type="text" class="form-control form-control-sm" name="jobcatergoryName" id="jobcatergoryName" placeholder="Job Category Name" autofocus="autofocus" value="<?= isset($Jobcategory->jobcatergoryName) ? $Jobcategory->jobcatergoryName : '' ?>" required="">
            </div>
        </div>
    </div>

    <div style="text-align: end;">
        <!-- Change the button type to "submit" for form submission -->
        <button type="button" id="btn_savee" class="btn samplebtn waves-effect waves-light">
            <i class="fa fa-check-circle" style="font-size: 20px; color: #FFF;"></i>Save
        </button>

        <a id="create" href="<?php echo base_url('Admin/Job/index'); ?>" class="Edit btn samplebtn waves-effect waves-light">
...
function saveperform()
{
    ...
    else
    {
      ...
      var formData = new FormData(form[0]);
            ...
            $.ajax({
        url:base_path+"Admin/job_update/updateJobCategory",
        type: "POST",
    ...
    {      
         // alert('Insert');
        $.ajax({
              url:base_path+"Admin/job_add/Add",
              type: "POST",
              data: $('#Form').serialize(),
              beforeSend: function(){
              $('#btn_savee').prop('disabled', true);
