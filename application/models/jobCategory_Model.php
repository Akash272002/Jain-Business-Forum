<?php
  class jobCategory_Model extends CI_Model {
    ...
    public function insertdata($model)
        {
          return $this->db->insert('jobcategory_master',$model);
            // return $sql->result();
        } 
     ...
     {      
      // SELECT studId, studentname, phone FROM student_master
             ...
             $this->db->select(" jobcategory_master.*");
             $this->db->from(' jobcategory_master');
              $query = $this->db->get();
              return $query->result();
     ...
     {
         $this->db->where('jobcatergoryId', $id);
         return $this->db->update('jobcategory_master', $data);
     }


     public function getJobCatById($id)
     {
         $this->db->where('jobcatergoryId', $id);
         $query = $this->db->get('jobcategory_master');
         return $query->row();
     }
